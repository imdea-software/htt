...
- (* i = n, reached the array end *)
  ...
  - (* swap happened in this iteration *)
    case=> H' Hlt; rewrite orbT; exists f'; split=>//.
    - rewrite Hf H'; apply/permEl/perm_catl.
      by rewrite !cat2s -perm_catCA.



    ffunE /= eqxx=>Hx.
      case: (path.splitPr Hx).
      (*apply/perm_iotaP=>/=. rewrite size_map size_enum_ord /=.
      Unset Printing Notations.
      rewrite -!map_comp.*)

    apply/permP=>p.
      rewrite count_map [RHS]count_map.
      apply: eq_in_count=>j Hj /=.

Definition bubble_sortT (a : {array 'I_n.+2 -> nat}) :=
  {f : {ffun 'I_n.+2 -> nat}},
  STsep (Array.shape a f,
...
Program Definition bubble_sort (a : {array 'I_n.+2 -> nat}) : bubble_sortT a :=
  Fix (fun (loop : bubble_sortT a) _ =>
       Do (sw <-- bubble_pass a;;
           if sw then loop tt else ret tt)).
